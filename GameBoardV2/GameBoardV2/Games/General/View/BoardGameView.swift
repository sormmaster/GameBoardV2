...
class BoardGameView: UIView {
    ...
    func setupColumns(value: String, y: Int, minX: CGFloat, minY: CGFloat, height: CGFloat, width: CGFloat, parent: BoardPieceDelegate) -> [BoardPieceView] {
        ...
        for index in 0 ... (rows - 1) {
            let frameY = (((CGFloat(y) * 2.0) + 1) * incrementHeight + minY)
            let frameX = (((CGFloat(index) * 2.0) + 1) * incrementWidth + minX)
            let frame = CGRect(x: frameX , y:frameY , width: incrementWidth, height: incrementHeight)
    ...
    func setupRows(parent: BoardPieceDelegate){
        for index in 0 ... (columns - 1) {
            boardViews.append(setupColumns(value: "-", y: index, minX: 0, minY: 0, height: frame.height, width: frame.width, parent: parent))
        }
    }
